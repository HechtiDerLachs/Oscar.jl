  ...
  function ToricCoveredScheme(ntv::NormalToricVariety)
    ...
    for i in 1:(length(patch_list)-1)
      ...
      for j in i+1:length(patch_list)
        ...
        Idext[l,l] = 0
        img_gens = [solve_mixed(AX, AY[:, k], Idext) for k in 1:ncols(AY)]
        ...
        Idext[l,l] = 0
        img_gens = [solve_mixed(AY, AX[:, k], Idext) for k in 1:ncols(AX)]
