...
function get_next_task2(N::GeometricGaussNode; depth::Int=0)
  #println(prod(" " for _ in 1:depth; init="") * "$(status(N))")
  status(N) == :done && return Tuple{Int, Int, Symbol}[], :done
  ...
  if status(N) == :explore_children
    ...
    i, j, _ = least_complex_entry(N)
    loc = N[i, j, :loc]
    loc_prog, loc_task = get_next_task2(loc; depth=depth+1)
    #println(prod(" " for _ in 1:depth; init="") * "loc_task: $(loc_task)")
    if !(loc_task == :wait || loc_task == :done)
      return push!(loc_prog, (i, j, :loc)), loc_task
    end
    sat = N[i, j, :quo]
    sat_prog, sat_task = get_next_task2(sat; depth=depth+1)
    if !(sat_task == :wait || sat_task == :done)
      return push!(sat_prog, (i, j, :quo)), sat_task
    end
    #println(prod(" " for _ in 1:depth; init="") * "quo_task: $(quo_task)")
    if loc_task == :done && sat_task == :done
      set_status!(N, :result_gathering)
      return Tuple{Int, Int, Symbol}[], :gather_results
    end
    return Tuple{Int, Int, Symbol}[], :wait
    (loc_task == :wait || loc_task == :done) && return push!(sat_prog, (i, j, :quo)), sat_task
    return push!(loc_prog, (i, j, :loc)), loc_task
  end
...
function _set_modulus!(R::MPolyQuoRing, g::Vector)
  ...
  ig.isReduced = false
  @req singular_poly_ring(P) === parent(singular_generators(ig)[1]) "singular ring mismatch"
...
#ideal_type(::Type{T}) where T <: MPolyQuoRing = MPolyQuoIdeal{elem_type(T)}

function _compute(task::GatherResults2)
  a = task.a
  loc_comps = task.loc_list
  quo_comps = task.quo_list
  R = parent(a)
  ...
  pb_loc_comps = Tuple{ideal_type(R), Int}[(saturation(I, a_ideal), r) for (I, r) in loc_comps]
  pb_quo_comps = Tuple{ideal_type(R), Int}[(I + a_ideal, r) for (I, r) in quo_comps]
  final_comps = Vector{Tuple{ideal_type(R), Int}}()
  @vprint :GeometricGauss 2 "ordering localized components...\n"
  final_comps = [(I, r) for (I, r) in pb_loc_comps if !is_one(I)]
  @vprint :GeometricGauss 2 "ordering quotient components...\n"
  for i in 1:length(pb_quo_comps)
    (I, r) = pb_quo_comps[i]
    #is_one(I) && continue
    container_found = false
    for (J, s) in final_comps
    ...
    !container_found && !is_one(I) && push!(final_comps, (I, r))
  end
  @vprint :GeometricGauss 2 "ordering quotient components...\n"
...
function _prepare_task2(N0::GeometricGaussNode, prog::Vector, symb::Symbol)
  N = N0[prog]
  #if symb == :compute_leaf
  #  return ComputeLeaf(matrix(N))
  if symb == :gather_results
    ...
    #@show prog
    loc_ideals = N[i, j, :loc].result::Vector
    sat_ideals = N[i, j, :quo].result::Vector
    R = base_ring(matrix(N))
    return GatherResults2(a, 
                          [(ideal(R, lifted_numerator.(gens(I))), r+1) for (I, r) in loc_ideals],
                          [(ideal(R, lifted_numerator.(gens(I))), r) for (I, r) in sat_ideals]
                         )
  elseif symb == :prepare_modulus
    R = base_ring(matrix(N))
    return PrepareModulus(R)
...
function _apply!(prog::Vector{Tuple{Int, Int, Symbol}}, A::SMat)
  ...
  elseif symb == :sat
    ...
    P = base_ring(I)::MPolyRing
    Q, pr = quo(P, I + ideal(P, c))
    @assert all(isone(lifted_denominator(x)) for x in A) "non-trivial denominators not allowed for saturated matrix"
    # TODO: Avoid anonymous function
    result = map_entries(x->Q(lifted_numerator(x)), A)
    _zero!(result[i], j)
    return _apply!(prog, result)
  else
...
_content(a::MPolyRingElem) = content(a)
_content(a::MPolyQuoRingElem) = content(lift(a))
_content(a::MPolyLocRingElem) = content(numerator(a))
_content(a::MPolyQuoLocRingElem) = content(lifted_numerator(a))

_gcd(a::T, b::T) where {T<:MPolyRingElem} = gcd(a, b)
_gcd(a::T, b::T) where {T<:MPolyQuoRingElem} = gcd(lift(a), lift(b))
_gcd(a::T, b::T) where {T<:MPolyLocRingElem} = gcd(numerator(a), numerator(b))
_gcd(a::T, b::T) where {T<:MPolyQuoLocRingElem} = gcd(lifted_numerator(a), lifted_numerator(b))
...
function _apply!(prog::Vector{Tuple{Int, Int, Symbol}}, A::MatrixElem)
  ...
  c = A[i, j]
  cont = _content(c)
  !is_one(cont) && multiply_row!(A, R(inv(cont)), i)
  m = nrows(A)
  n = ncols(A)
  if symb == :loc
    L, loc = localization(R, powers_of_element(lifted_numerator(c)))
    ...
    for k in 1:nrows(A)
      ...
      LA = AbstractAlgebra.multiply_row!(LA, divexact(lc, g), k)
      AbstractAlgebra.add_row!(LA, -divexact(b, g), i, k)
    end
  ...
  elseif symb == :sat
    I = saturated_ideal(modulus(R))
    P = base_ring(I)::MPolyRing
    Q, pr = quo(P, I + ideal(P, lifted_numerator(c)))
    @assert all(isone(lifted_denominator(x)) for x in A) "non-trivial denominators not allowed for saturated matrix"
    # TODO: Avoid anonymous function
    result = map_entries(x->Q(lifted_numerator(x)), A)
    result[i, j] = zero(Q)
...
@doc raw"""
    rank_stratification(A::MatrixElem)

Given a matrix ``A ∈ Rᵐˣⁿ`` over a commutative ring `R`, 
let ``X := Spec R = ⋃ ᵣ Vᵣ`` be the decomposition into 
disjoint, locally closed subsets ``V_r`` of loci where ``A`` 
has rank precisely ``r``. This function returns a list 
of pairs `(I, r)` where ``I ⊂ R`` is an ideal such that 
the zero locus ``V(I) ⊂ Spec R`` is the closure of a locally 
dense open subset ``U ⊂ Vᵣ`` and ``X`` is the union of 
these ``V(I)``. 
"""
function rank_stratification(A::MatrixElem; distributed::Bool=false)
  !distributed && return _rank_stratification_local(GeometricGaussNode(A))
# try 
#   workers()
# catch e
...
# end
  ...
  while true
    # assign work
    while any(isnothing, futures)
      @vprint :GeometricGauss 2 "assigning work...\n"
      i = findfirst(isnothing, futures)
      @vprint :GeometricGauss 2 "worker $i is free\n"
      fut = futures[i]
      #@show i, isnothing(fut)
      id = worker_ids[i]
      #println("computing next task...")
      prog, task = get_next_task2(N0) 
      @vprint :GeometricGauss 2 "next task: $prog, $task\n"
      ...
      if task == :done
        all(isnothing, futures) || error("not all tasks are done")
        @vprint :GeometricGauss 2 "all tasks accomplished; finishing\n"
        return N0.result
      end
      @vprint :GeometricGauss 2 "calling task on worker $i\n"
      N = N0[prog]
      assigned_tasks[i] = (prog, task)
      par_task = _prepare_task2(N0, prog, task)
      #@show typeof(par_task)
      put_type_params(channels[i], par_task)
      futures[i] = remotecall(_compute, id, par_task)
    end

    @vprint :GeometricGauss 2 "cleaning up...\n"
    i = findfirst(x->(!isnothing(x) && isready(x)), futures)
    i === nothing && continue
    fut = futures[i]
    prog, task = assigned_tasks[i]
    @vprint :GeometricGauss 2 "worker $i has a result for $prog, $task\n"
    #@show task
    N = N0[prog]
    if task == :prepare_modulus
      #@show "modulus prepared"
    ...
    futures[i] = nothing # make worker available
    assigned_tasks[i] = nothing 
    @vprint :GeometricGauss 2 "done processing result\n"
  end
  #@show isnothing.(futures)
  #@show [!isnothing(fut) && isready(fut) for fut in futures]
  if status(N0) == :done
...
function _rank_stratification_local(N0::GeometricGaussNode; depth::Int=0)
  offset = prod("  " for _ in 1:depth; init = "")
  A = matrix(N0)
  @vprint :GeometricGauss 2 offset*"exploring node for matrix of size $(size(A))\n"
  #println(offset*"$A")
  R = base_ring(A)
  #println(offset * "$(R)")
  is_zero(A) && return [(ideal(R, elem_type(R)[]), 0)]
  i, j, c, _ = least_complex_entry(N0)
  @vprint :GeometricGauss 2 offset*"least complex entry: ($i, $j) -> $(_complexity(c))\n"
  #println(offset*"$i, $j, $c")
  #quo_node = N0[i, j, :quo]
  sat_node = N0[i, j, :sat]
  loc_node = N0[i, j, :loc]
  sat_res = [(ideal(R, [lifted_numerator(x) for x in gens(K)]) + ideal(R, c), r) for (K, r) in _rank_stratification_local(sat_node; depth=depth+1)]
  @vprint :GeometricGauss 2 offset*":sat result:\n"
  for (I, r) in sat_res
    @vprint :GeometricGauss 2 offset*"  $r: $(ngens(I))\n"
  end
 # println(offset*"sat:")
 # for (I, r) in sat_res
 #   println(offset*"$r : $(gens(I))")
 # end
  loc_res = [(saturation(ideal(R, [lifted_numerator(x) for x in gens(I)]), ideal(R, c)), r+1) for (I, r) in _rank_stratification_local(loc_node; depth=depth+1)]
  @vprint :GeometricGauss 2 offset*":loc result:\n"
  for (I, r) in loc_res
    @vprint :GeometricGauss 2 offset*"  $r: $(ngens(I))\n"
  end
 # println(offset*"loc:")
 ...
 #   println(offset*"$r : $(gens(I))")
 # end
  result = Tuple{ideal_type(R), Int}[]
  for (I, r) in loc_res
    matched = Int[]
    for (i, (K, s)) in enumerate(sat_res)
      # If V(K) is in the closure of V(I) with the same rank, then discard it. 
      s == r || continue
      if is_subset(I, K) # requires only one gb computation for K
        push!(matched, i) # component is memorized to be discarded
        continue
      end
      is_one(I + K) && continue # components do not even touch
      if all(radical_membership(x, K) for x in gens(I)) # another, more expensive containment test
        push!(matched, i)
      end
    end
    sat_res = [(I, r) for (i, (I, r)) in enumerate(sat_res) if !(i in matched)]
  end
  return vcat(loc_res, sat_res)
end
